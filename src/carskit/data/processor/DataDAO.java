...
import carskit.data.structure.Triple;
...
public class DataDAO {
    ...
    private static double[][] coOcuuranceItems;
    private static double[][] coOccuranceContextItems;
    private double[][] coOccuranceContextUsers;
    private static double[][][] coOccuranceTensor;
    private static Multimap<Integer, Integer> coOccuranceList;
    ...
    private Multimap<Integer, Integer> uRatedList, iRatedList, dimConditionsList, condContextsList, condContext_ItemsList, condContext_UsersList;

    private HashMap<Integer, Integer> uiUserIds, uiItemIds, condDimensionMap;
    private HashMap<Integer, ArrayList<Integer>> dimensionConditionsList, contextConditionsList, uiConditionsList;

    private ArrayList<Integer> EmptyContextConditions;

    private double[][] trainMtrix;
    ...
    public DataDAO(String path, BiMap<String, Integer> userIds, BiMap<String, Integer> itemIds, BiMap<String, Integer> ctxIds, BiMap<String, Integer> uiIds,
        ...
        this.coOccuranceList = HashMultimap.create();
        this.condContext_ItemsList = HashMultimap.create();
        this.condContext_UsersList = HashMultimap.create();
       // this.coOcuuranceItems= new HashMap<String,Integer>();
     ...
     * this feature, i.e., keep the original rating value, set the threshold a
     * negative value
    ...
    public SparseMatrix readData(double binThold) throws Exception {
        ...
        //add by myself
        if (EmptyContextConditions.size() == 0 && data.length > 3) {

            for (int i = 0; i < dimIds.size(); i++) {
                EmptyContextConditions.add(i);
            }
        }
        ...
        while ((line = br.readLine()) != null) {
            ...
            for (int i = 3; i < data.length; ++i) {
                ...
                if (data[i].trim().equals("")) {
                    // Logs.error(counter_line+":"+line+"; data[i]="+data[i]);
                    //counter_line++;
                    break;
                } else {
            ...
            for (Integer cond : condList) {
                ...
                this.condContext_ItemsList.put(cond, col);
                this.condContext_UsersList.put(cond, row);
            }
        ...
        Logs.debug(" line_number={}", counter_line);
        br.close();
        ...
        Logs.debug("Rating data set has been successfully loaded. .. ");

      //  setCoOccuranceItems();
        System.out.println("**********print UserIds:");
        Object[] keys = userIds.keySet().toArray();
        /*for (Object key : keys) {
            Object entry = userIds.get(key);
            System.out.println("user:" + key.toString() + ", id:" + entry.toString());
        }

        System.out.println("**********print itemIds:");
        Object[] keys2 = itemIds.keySet().toArray();
        for (Object key : keys2) {
            Object entry = itemIds.get(key);
            System.out.println("item:" + key.toString() + ", id:" + entry.toString());
        }*/
    ...
    public void setCoOccuranceItems() {

        coOcuuranceItems = new double[this.numItems()][this.numItems()];
        Set keySet = this.iRatedList.keySet();
        Iterator keyIterator = keySet.iterator();

        Collection values = null;
        Iterator valuesIterator = null;
        ArrayList<ArrayList<Integer>> iRatedList_arrays = new ArrayList<ArrayList<Integer>>();
        while (keyIterator.hasNext()) {
            Integer col_key = (Integer) keyIterator.next();

        //System.out.print( "Key: " + col_key + ", " );
            values = (Collection) iRatedList.get((Integer) col_key);
            valuesIterator = values.iterator();
            ArrayList<Integer> vaules_array = new ArrayList<Integer>();
            while (valuesIterator.hasNext()) {
                Integer val = (Integer) valuesIterator.next();
                vaules_array.add(val);
            // System.out.print( "vaule: " + val + ", " );

            }
            iRatedList_arrays.add(col_key, vaules_array);
            System.out.print("\n");
        }
        ...
        valuesIterator = null;
        double max_value = -1;
        for (int i = 0; i < iRatedList_arrays.size(); i++) {
            ArrayList<Integer> list_coli = iRatedList_arrays.get(i);
            //  String item_id= idItems.get(i);
            if (i % 1500 == 0) {
                System.out.print(" key _i: " + i + ", ");
            }

            for (int j = i + 1; j < iRatedList_arrays.size(); j++) {
                ArrayList<Integer> list_colj = iRatedList_arrays.get(j);
                // System.out.print( " key _j: "+j + ", " );
                for (int k = 0; k < list_coli.size(); k++) {
                    Integer ui_i = list_coli.get(k);

                 //  String user_item=idUIs.get(val_j);
                    // String user_j=user_item.split("_")[0];
                    int row_i = uiUserIds.get(ui_i);
                    int col_i = uiItemIds.get(ui_i);
                    // System.out.print("vaule-i : "+list_coli.get(k)+ ", " );
                    for (int x = 0; x < list_colj.size(); x++) {
                        // System.out.print("vaule-j : "+list_colj.get(x)+ ", " );
                        Integer ui_j = list_colj.get(x);
                        int row_j = uiUserIds.get(ui_j);
                        int col_j = uiItemIds.get(ui_j);

                        if (row_i == row_j) {
                            //  String item_i=idItems.get(val_i);
                            //String item_j=idItems.get(val_j);
                            coOcuuranceItems[col_i][col_j]++;
                            if (coOcuuranceItems[col_i][col_j] > max_value) {
                                max_value = coOcuuranceItems[col_i][col_j];
                            }
                        }
        ...
        System.out.println("co-occurance items");
        // scale the numbers in this.coOcuuranceItems:

        for (int i = 0; i < coOcuuranceItems.length; i++) {

            System.out.println("\n i=" + i);
            for (int j = 0; j < coOcuuranceItems.length; j++) {

                if (coOcuuranceItems[i][j] != 0) {
                    coOccuranceList.put(j, i);
                }
                coOcuuranceItems[i][j] /= max_value;

                coOcuuranceItems[j][i] = coOcuuranceItems[i][j];
                System.out.print(coOcuuranceItems[i][j] + ",");
    ...
    public double[][] getCoOccuranceItems() {
        return this.coOcuuranceItems;
    }

    public HashMultimap<Integer, Integer> getCoOccuranceItemsList() {
        return (HashMultimap<Integer, Integer>) this.coOccuranceList;
    }

    public void setContextCoOccuranceuItems() {

        this.coOccuranceContextItems = new double[this.numConditions()][this.numConditions()];
        Set keySet = this.condContext_ItemsList.keySet();
        Iterator keyIterator1 = keySet.iterator();

        Collection values = null;
        Iterator valuesIterator = null;

        ArrayList<ArrayList<Integer>> ccItems_list = new ArrayList<ArrayList<Integer>>();
        while (keyIterator1.hasNext()) {
            Integer col_key = (Integer) keyIterator1.next();

       // System.out.print( "Key: " + col_key + ", " );
            values = (Collection) condContext_ItemsList.get((Integer) col_key);
            valuesIterator = values.iterator();
            ArrayList<Integer> vaules_array = new ArrayList<Integer>();
            while (valuesIterator.hasNext()) {
                Integer val = (Integer) valuesIterator.next();
            ...
            ccItems_list.add(col_key, vaules_array);
            System.out.print("\n");
        }
        ...
        double max_value = -1;
        for (int i = 0; i < ccItems_list.size(); i++) {
            ArrayList<Integer> list_coli = ccItems_list.get(i);
            //  String item_id= idItems.get(i);

            for (int j = i + 1; j < ccItems_list.size(); j++) {
                ArrayList<Integer> list_colj = ccItems_list.get(j);
                // System.out.print( " key _j: "+j + ", " );
                for (int k = 0; k < list_coli.size(); k++) {
                    Integer col_i = list_coli.get(k);

                    for (int x = 0; x < list_colj.size(); x++) {
                        // System.out.print("vaule-j : "+list_colj.get(x)+ ", " );
                        Integer col_j = list_colj.get(x);

                        if (col_i == col_j) {
                            //  String item_i=idItems.get(val_i);
                            //String item_j=idItems.get(val_j);
                            this.coOccuranceContextItems[i][j]++;
                            if (this.coOccuranceContextItems[i][j] > max_value) {
                                max_value = this.coOccuranceContextItems[i][j];
                            }
                        }
        ...
        System.out.println("coOccuranceContextItems:");
          // scale the numbers in this.coOcuuranceItems:

        for (int i = 0; i < coOccuranceContextItems.length; i++) {

            System.out.println("\n i=" + i);
            for (int j = 0; j < coOccuranceContextItems.length; j++) {

                if (coOccuranceContextItems[i][j] != 0) {
                    // this.coOccuranceContextItems.put(j, i);
                }
                this.coOccuranceContextItems[i][j] /= max_value;
                this.coOccuranceContextItems[j][i] = this.coOccuranceContextItems[i][j];
                System.out.print(this.coOccuranceContextItems[i][j] + ",");
    ...
    public double[][] getCoOccuranceContextItems() {
        return this.coOccuranceContextItems;
    }

    public void setContextCoOccuranceUsers() {

    }

    public void writeFileDataMatrix(SparseMatrix matrix) {
        Table<Integer, Integer, Double> dataTable = matrix.getDataTable();

        double[][] X = new double[943][1682];

        Set<Table.Cell<Integer, Integer, Double>> cells = dataTable.cellSet();

        Iterator iter = cells.iterator();
        while (iter.hasNext()) {
            String line = iter.next().toString();
            String uic_str = line.split(",")[0];
            int uic = Integer.parseInt(uic_str.substring(1));

            System.out.println("***" + line);
            int row = uiUserIds.get(uic);
            int col = uiItemIds.get(uic);
       ...
       // train = numUsers=943, numItems=1663
        //test -numItems:1682, -numUsers:943
        System.out.println("numUsers=" + this.numUsers() + ", numItems=" + this.numItems());

        try {
            FileWriter fw = new FileWriter("matrix_userItem_test.txt");
    ...
    public void setCoOccuranceTensor() {

        System.out.println("****** setCoOccuranceTensor*******");
        Set keySet = this.uiConditionsList.keySet();
        Iterator keyIterator1 = keySet.iterator();

        Triple[] triples = new Triple[uiConditionsList.size()];
        int counter = 0;
        for (Map.Entry<Integer, ArrayList<Integer>> entrySet : uiConditionsList.entrySet()) {
            Integer key_ui = entrySet.getKey();
            ArrayList<Integer> condlist = entrySet.getValue();
            triples[counter] = new Triple((Integer) uiUserIds.get(key_ui), (Integer) uiItemIds.get(key_ui), condlist);
            triples[counter].condlist=condlist;
            counter++;
        }

        coOccuranceTensor = new double[numItems()][numItems()][numUsers()];
        // max = max size cond list of first dimention
        int max_cond_size = dimensionConditionsList.get(0).size();
        for (int i = 0; i < triples.length; i++) {
            for (int j = i + 1; j < triples.length; j++) {
                if (triples[i].user == triples[j].user && triples[i].item != triples[j].item) {

                    int subscribe = 0;
                    for (int c1 = 0; c1 < triples[i].condlist.size(); c1++) {
                        for (int c2 = 0; c2 < triples[j].condlist.size(); c2++) {
                            if (triples[i].condlist.get(c1) == triples[j].condlist.get(c2)) {
                                subscribe++;
                                break;
                            }
                        }
                    ...
                    coOccuranceTensor[triples[i].item][triples[j].item][triples[i].user] = (double) subscribe / max_cond_size;

                    coOccuranceTensor[triples[j].item][triples[i].item][triples[i].user] = (double) subscribe / max_cond_size;

                }

                if (triples[i].user == triples[j].user && triples[i].item == triples[j].item) {
                    coOccuranceTensor[triples[i].item][triples[i].item][triples[i].user] = 0.0;
    ...
    public void writeCoOccuranceTensorToFile(String filename) {
        ...
        try {
            ...
            System.out.println();
            //k=depth
            for (int k = 0; k < this.numUsers(); k++) {

                //j=row
                for (int j = 0; j < this.numItems(); j++) {
                    //i=col
                    for (int i = 0; i < this.numItems(); i++) {
                        fw.write(coOccuranceTensor[i][j][k] + ",");
                        System.out.print(coOccuranceTensor[i][j][k] + ",");
                    }
                    fw.write("\n");
                    System.out.println();
    ...
    public void readCoOccuranceTensorFromFile(String filename){
        
        System.out.println("*********readCoOccuranceTensorFromFile");
             Logs.debug(" readCoOccuranceTensorFromFile");     
        this.coOccuranceTensor= new double[numItems()][numItems()][numUsers()];
                ...
                int u=0;
                 ...
                 if(!line.contains("**")){
                     ...
                     for( i=0;i<data.length;i++){
                         coOccuranceTensor[i][j][u]=Double.parseDouble(data[i]);
                        // System.out.print(coOccuranceTensor[i][j][u]+",");
                     }
                     j++;
                     //System.out.println();
                 }
    ...
    public double[][][] getCoOccuranceTensor(){
        return this.coOccuranceTensor;
    }
    ...
    public SparseTensor toSparseTensor(SparseMatrix sm) {
        ...
        for (librec.data.MatrixEntry me : sm) {
            ...
            // System.out.print(uid+": "+this.getUserId(uid)+",");
            //System.out.print(iid+": "+this.getItemId(iid)+",");
            ...
            for (int condId : listOfConditions) {
               // System.out.print("condId:"+condId+", "+"="+this.getContextConditionId(condId)+",  ");

                if (uiConditionsList.containsKey(rowid)) {
                    ArrayList<Integer> list = uiConditionsList.get(rowid);
                    if (list.indexOf(condId) == -1) {
                        list.add(condId);
                    }
                    uiConditionsList.put(rowid, list);
                } else {
                ...
                //System.out.print("index_condId:"+index_condId+", ");
            }
            // System.out.println();
        }
        for (int d = 0; d < numDims; d++) {
     ...
     *
     * List<String> lines = new ArrayList<>(1500); for (MatrixEntry me :
     * rateMatrix) { String line = Strings.toString(new Object[] { me.row() + 1,
     * me.column() + 1, (float) me.get() }, sep); lines.add(line);
     *
     * if (lines.size() >= 1000) { FileIO.writeList(toPath, lines, null, true);
     * lines.clear(); } }
     *
     * if (lines.size() > 0) FileIO.writeList(toPath, lines, null, true);
     *
     ...
     *
     * public void writeData(String toPath) throws Exception { writeData(toPath,
     * " "); }
     */
    ...
    /**
     * @param rawId raw user-item id as String
    ...
    /**
     * @param rawUserId raw user id as String
     * @param rawItemId raw item id as String
     * @return inner user-item id as int
     */
    ...
    /**
     * @param innerId inner user-item id as int
    ...
    /**
     * @param innerId inner condition id as int
    ...
    /**
     * @param innerId inner dimension id as int
    ...
    /**
     * @param rawId raw dimension id as String
     * @return inner dimension id as int
     */
    ...
    /**
     * @param rawId raw condition id as String
    ...
    /**
     * @param rawId raw context id as String
    ...
    /**
     * @param innerId inner context id as int
