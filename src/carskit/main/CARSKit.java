...
import carskit.alg.cars.adaptation.independent.BiasedCPTF;
import carskit.alg.cars.adaptation.independent.BiasedCPTF_v2;
...
public class CARSKit {
    ...
    protected static String defaultConfigFileName = "setting-Tijuanan_returant.conf";
    ...
    public static int num_factor,num_max_iter;
    ...
    protected String dataset_algorithm;
    public static String datasetName;

    public static void main(String[] args) throws Exception {

        System.out.println("CARSKit.main");
        double x=0.17834580934509834;
        double y=0.23453549890830948;
        //        0.41288130825340782
        System.out.println("value="+(x+y));// real=0.412881308253 , printed=0.41288130825340785
        //System.exit(0);
        //new CARSKit().writeResultsToCSV();
        //System.exit(1);
        try {
    ...
    public static void setNumFactor_NumIter_FromFile(){
        try {
            String file="factor_iteration.txt";
            ...
            try {
                ...
                num_factor=Integer.parseInt(str.split(",")[0]);
                num_max_iter=Integer.parseInt(str.split(",")[1]);
                
                fileReader.close();
            } catch (IOException ex) {
                Logger.getLogger(CARSKit.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public static void writeNumFactor_NumIter_ToFile(){
        
         FileWriter fw = null;
       
        try {
            fw = new FileWriter("factor_iteration.txt");
            fw.write(num_factor+","+num_max_iter);
            fw.close();
        } catch (IOException ex) {
            Logger.getLogger(CARSKit.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    protected void execute(String[] args) throws Exception {
        System.out.println("CARSKit.execute");
        ...
        for (String config : configFiles) {
            ...
            //num.max.iter
            cf = new FileConfiger(config);

            String[] filename_part = cf.getString("dataset.ratings.wins").split("\\\\");
             datasetName=filename_part[filename_part.length-2];
               dataset_algorithm=datasetName+"_"+cf.getString("recommender");
            //max=20;
            //max_iter=200;
              
                setNumFactor_NumIter_FromFile();
            for ( ;num_factor <=20; num_factor++) {
                //for (int num_neighbors = 1; num_neighbors < 3; num_neighbors++) {
                    for (; num_max_iter <= 200; num_max_iter++) {
                        ...
                        cf.setString("num.factors", String.valueOf(num_factor));
                       // cf.setString("num.neighbors", String.valueOf(num_neighbors));
                        cf.setString("num.max.iter", String.valueOf(num_max_iter));
                        System.out.println("num.neighbors=" + cf.getString("num.neighbors"));//10
                        System.out.println("num.fators=" + cf.getString("num.factors"));//50
                        System.out.println("recommender=" + cf.getString("recommender"));//
                        preset_new();
...
"results.txt", results);
   ...
   // writeResultsToCSV();
    ...
    public  void writeResultsToCSV(){
        FileWriter fw = null;
        dataset_algorithm="D:\\CUR\\RecommenderSystems\\Thesis\\Resturant_Tijuna_location_ratio\\Restaurant_TijuanaRestaurant_CPTF_locatio_ratio";
        try {
            FileReader fileReader = null;
            fw = new FileWriter(dataset_algorithm+".csv");
            fileReader = new FileReader(dataset_algorithm+".txt");
            BufferedReader reader = new BufferedReader(fileReader);
            StringBuilder write_header = new StringBuilder();
            ...
            fw.write(write_header.toString()+"\n");
    ...
    protected void setConfiger(String configFile) throws Exception{
        cf = new FileConfiger(configFile);
        String separator=System.getProperty("file.separator");
    ...
    protected void preset_new() throws Exception {
    ...
    protected void readData() throws Exception {
        // DAO object
         ...
         System.out.println("CARSKit.readData");
      ...
      rateDao.readCoOccuranceTensorFromFile(datasetName+"_location_coOccuranceTensor.txt");
     // rateDao.setCoOccuranceItems();
     // rateDao.setContextCoOccuranceuItems();
        System.out.println("writing trainData:");
       // rateDao.writeFileDataMatrix(rateMatrix);
       //System.exit(1);
        rateDao.printSpecs();
    ...
    protected void runAlgorithm() throws Exception {
         ...
         System.out.println("CARSKit.runAlgorithm");
        // evaluation setup
        String setup = cf.getString("evaluation.setup");
        LineConfiger evalOptions = new LineConfiger(setup);
        ...
        switch (evalOptions.getMainParam().toLowerCase()) {
            ...
            case "test-set":
               ...
               System.out.println("test -numItems:"+testDao.numItems()+", -numUsers:"+testDao.numUsers()); 
                System.out.println("writing test Data:");
                
               // testDao.writeFileDataMatrix(testData);
                 //System.exit(1);
    ...
    private void printEvalInfo(Recommender algo, Map<Measure, Double> ms) throws Exception {
        ...
        String evalInfo = String.format("Final Results by %s, %s, %s, %s", algo.algoName, result, algo.toString(), time,
        ...
        if (outputOptions.contains("--to-file")) {
          //  String filePath = outputOptions.getString("--to-file", WorkingPath + algorithm + ".txt");
            //String filePath = outputOptions.getString("--to-file", WorkingPath + dataset_algorithm + ".txt");
            String filePath=dataset_algorithm + ".txt";
    ...
    private void printEvalInfo_CSVFile(Recommender algo, Map<Measure, Double> ms) throws Exception {
        ...
        String evalInfo = String.format("Final Results by %s, %s, %s, Time: %s%s", algo.algoName, result, algo.toString(), time,
                (outputOptions.contains("--measures-only") ? "" : "\n"));
        FileWriter fw = new FileWriter(dataset_algorithm+".csv");
        
       
        String[] algo_array=algo.toString().split(",");
    ...
    private void runCrossValidation(LineConfiger params) throws Exception {
         ...
         System.out.println("CARSKit.runCrossValidation");
        ...
        for (Recommender algo : algos) {
            ...
            /*finally{
                 System.out.println("size algo="+algo.measures.size());
                System.exit(0);
                }*/
    ...
    protected Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
        ...
        System.out.println("proteted Recommender getRecommender");
        ...
        switch (algOptions.getMainParam().toLowerCase()) {
            ...
            case "pmf":
                return new PMF_orginal(trainMatrix, testMatrix, fold);
            case "bpmf":
            ...
            case "bpr":
                return new BPR_orginal(trainMatrix, testMatrix, fold);
            case "lrmf":
            ...
            case "usersplitting":
            {
                ...
                switch(recsys_traditional)
                {
                    ...
                    case "pmf":
                        recsys=new PMF_orginal(trainMatrix, testMatrix, fold);break;
                    case "bpmf":
                    ...
                    case "bpr":
                        recsys=new BPR_orginal(trainMatrix, testMatrix, fold);break;
                    case "lrmf":
            ...
            case "biasedcptf": {
                rateDao.LoadAsTensor();
               // rateDao.writeCoOccuranceTensorToFile(datasetName);
                //rateDao.readCoOccuranceTensorFromFile(datasetName+"_location_coOccuranceTensor.txt");
                return new BiasedCPTF(trainMatrix, testMatrix, fold);
            }
             case "biasedcptf_v2": {
                rateDao.LoadAsTensor();
               // rateDao.writeCoOccuranceTensorToFile(datasetName);
                //calling here cause run for every thread
               // rateDao.readCoOccuranceTensorFromFile(datasetName+"_location_coOccuranceTensor.txt");
                return new BiasedCPTF_v2(trainMatrix, testMatrix, fold);
            }
            ...
            case "itemembedding_v4":
                return new ItemEmbedding_v4(trainMatrix,testMatrix,fold,this.rateDao.getCoOccuranceItems(),this.rateDao.getCoOccuranceContextItems());
