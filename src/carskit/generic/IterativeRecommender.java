...
public abstract class IterativeRecommender extends Recommender {
    ...
    protected static float regU, regI, regB, reg, regC,reg_lam_gamma=(float) 1e+0;
    // number of factors
    ...
    // objective loss
    protected double loss, last_loss,loss_itemEmbedding,loss_MF=0;
    ...
    protected boolean isConverged(int iter) throws Exception {
        ...
        if (verbose) {
           ...
           // Logs.debug("{}{} iter {}: loss = {},loss_embedding={},loss_MF={}, delta_loss = {}{}{}", algoName, foldInfo, iter,
             //       (float) loss, loss_itemEmbedding,loss_MF,delta_loss, earlyStop, learnRate);
            
            Logs.debug("{}{} , numFactor={} , num_Max_iter={}, iter {}: loss = {}, delta_loss = {}{}{} , ", algoName, foldInfo,
                    carskit.main.CARSKit.num_factor,carskit.main.CARSKit.num_max_iter, iter,
                    (float) loss, loss_itemEmbedding,delta_loss, earlyStop, learnRate);
            
        }

        if (Double.isNaN(loss) || Double.isInfinite(loss)) {
            Logs.error("Loss = NaN or Infinity: current settings does not fit the recommender! Change the settings and try again!");
            carskit.main.CARSKit.writeNumFactor_NumIter_ToFile();
            //System.exit(0);
            throw(new Exception());
        }
