...
import static carskit.generic.ContextRecommender.numConditions;
...
public class TensorRecommender extends IterativeRecommender {
    ...
    public String toString() {
        ...
        return Strings.toString(new Object[] { "numFactors: "+numFactors,"numIter: "+numIters, "lrate: "+initLRate, "maxlrate: "+maxLRate, "reg: "+reg,   "isBoldDriver: "+isBoldDriver });
    }
    ...
    public int[] getKeys(int u, int i, int c){
        ...
        for(int k=0;k<conds.size();++k){
            int condId = conds.get(k);
             int index=0;
            try{
             index = dimensionConditionsList.get(++start).indexOf(condId);
